...
class Buyback extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $data = [
      ...
      'content'                       => 'admin/buyback/index_buyback'
    ];
  ...
  public function create()
  {
    ...
    $this->form_validation->set_rules(
      'buyback_name','Nama produk','required',
      [
        'required'                    => 'Nama produk harus di isi',
      ]
    );
    $this->form_validation->set_rules(
      'buyback_desc','Deskripsi Produk','required',
      [
    ...
    if ($this->form_validation->run()) {
      ...
      if (!$this->upload->do_upload('buyback_img')) {
        //End Validasi
        $data = [
          'title'                     => 'Tambah Buyback',
          'category_buy'              => $category_buy,
          'error_upload'              => $this->upload->display_errors(),
          'content'                   => 'admin/buyback/create_buyback'
        ];
        $this->load->view('admin/layout/wrapp', $data, FALSE);
        //Masuk Database
      } else {
        ...
        $buyback_slug  = url_title($this->input->post('buyback_name'), 'dash', TRUE);
        $data  = [
          'user_id'                   => $this->session->userdata('id'),
          'category_buy_id'           => $this->input->post('category_buy_id'),
          'buyback_slug'              => $slugcode . '-' . $buyback_slug,
          'buyback_name'              => $this->input->post('buyback_name'),
          'buyback_desc'              => $this->input->post('buyback_desc'),
          'buyback_price'             => $this->input->post('buyback_price'),
          'buyback_size'              => $this->input->post('buyback_size'),
          'buyback_img'               => $upload_data['uploads']['file_name'],
          'buyback_status'            => $this->input->post('buyback_status'),
          'date_created'              => time()
        ];
        $this->buyback_model->create($data);
        $this->session->set_flashdata('message', 'Data Produk telah ditambahkan');
        redirect(base_url('admin/buyback'), 'refresh');
      }
    }
    //End Masuk Database
    $data = [
  ...
  public function Update($id)
  {
    ...
    if ($valid->run()) {
      ...
      if (!empty($_FILES['buyback_img']['name'])) {
        $config['upload_path']          = './assets/img/product/';
        $config['allowed_types']        = 'gif|jpg|png|jpeg';
        $config['max_size']             = 5000; //Dalam Kilobyte
        $config['max_width']            = 5000; //Lebar (pixel)
        ...
        if (!$this->upload->do_upload('buyback_img')) {
          ...
          $data = [
            ...
            'buyback'                   => $buyback,
            'error_upload'              => $this->upload->display_errors(),
            'content'                   => 'admin/buyback/update_buyback'
          ];
          $this->load->view('admin/layout/wrapp', $data, FALSE);
          //Masuk Database
        } else {
          ...
          if ($buyback->buyback_img != "") {
            unlink('./assets/img/product/' . $buyback->buyback_img);
          ...
          $this->buyback_model->update($data);
  ...
  public function delete($id)
  {
    ...
    //End Hapus Gambar
    $data = ['id'                         => $buyback->id];
    $this->buyback_model->delete($data);
